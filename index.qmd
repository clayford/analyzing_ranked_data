...
`mar` component shows number of times where item i (row) is ranked j (column). We can add row names to make thsi easier to read.
...
Look at MDS coefficients for raters 2 - 4 and their rankings.
...
Can use `psychotools::itempar()` to estimate probabilities that each item received the highest rank.
